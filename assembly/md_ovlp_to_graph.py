...
def identify_spurs(ug, u_edge_data):
    ...
    while len(s_candidates) > 0:
        ...
        for b_node in n_ego_graph.nodes():
            ...
            if total_length >= 30000:   # Note: changed from 50000 to 30000 in pbmga
                continue
...
def construct_c_path_from_utgs(ug, u_edge_data, sg):
    ...
    while free_edges:
        ...
        for s, t, v in ug.out_edges(n, keys=True):
            ...
            path_key = t
            t0 = s
            while t in simple_nodes: #simple_out:   # Note: change for generating not contigs but unitigs in pbmga
                if t in path_nodes:
                    break
                rt = reverse_end(t)
                if rt in path_nodes:
...
def ovlp_to_graph(args):
    ...
    for path_start, path_key, path_end, p_len, p_score, path, n_edges in c_path:
        cg.add_edge(path_start, path_end)
    nx.write_gexf(cg, "unitig_graph.gexf")

    cg_assert = nx.DiGraph()
    ...
    for path_start, path_key, path_end, p_len, p_score, path, n_edges in c_path:
        ...
        cg_assert.add_edge(s0, end_node)

        print >> ctg_paths, "%06dF" % ctg_id, "utg_linear", s0+"~"+v0+"~"+t0, end_node, length, score, "|".join([ c[0]+"~"+c[2]+"~"+c[1] for c in non_overlapped_path ] )
        ...
        print >> ctg_paths, "%06dR" % ctg_id, "utg_linear", s0+"~"+v0+"~"+t0, end_node, length_r, score_r, "|".join([ c[0]+"~"+c[2]+"~"+c[1] for c in non_overlapped_path_r ] )
        ctg_id += 1
    ...
    nx.write_gexf(cg_assert, "unitig_graph.gexf.assert")
                
    for s, t, v in list(circular_path):
        length, score, path, type_ = u_edge_data[ (s, t, v) ]
        print >> ctg_paths, "%6d" % ctg_id, "utg_circular", s+"~"+v+"~"+t, t, length, score, s+"~"+v+"~"+t
        ctg_id += 1
